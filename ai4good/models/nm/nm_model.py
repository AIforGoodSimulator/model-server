...
def run_parallel(p, sampled_graph, node_groups):
    lazy_sols = []
    for ii in range(10):
        if "interventions" in p.profile_name:
                lazy_result = dask.delayed(process_graph_interventions)(p, sampled_graph, node_groups)
        else:
            lazy_result = dask.delayed(process_graph)(p, sampled_graph, node_groups)
