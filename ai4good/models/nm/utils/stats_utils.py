...
def increase_population(old_pop, new_pop, pop_df_file, **kwargs):
    ...
    new_pop_ranges = np.random.choice(
        range(len(age_ranges)), p=age_prevalence, size=diff_pop, replace=True
    )
    new_pop_sex = np.random.choice(
        [0, 1], p=sex_prevalence, size=diff_pop, replace=True
    )
    ...
    for i in range(diff_pop):
        ...
        if "70+" in age_range:
            ...
            person_age = np.random.choice(
                range(int(age_range[0]), int(age_range[1]) + 1)
            )
...
def sample_population(n_sample, pop_df_file):
    ...
    pop_df["prob_symptomatic"] = pop_df.apply(
        lambda row: get_prob_symptomatic(row), axis=1
    )
    pop_df["prob_hospitalisation"] = pop_df.apply(
        lambda row: get_prob_hospitalisation(row), axis=1
    )
